...
class Bitcoind(BitcoindDerivate):
    ...
    def __init__(self, coin, url):
        ...
        # Bitcoind has its internal accounting.
        # We put all balances on this specific account
    ...
    def send(self, recipients):
        ...
        txid = self.api_call("sendmany", self.bitcoind_account_name, amounts)
...
class TransactionUpdater:
    ...
    def __init__(self, session, backend, wallet_class, wallet_id):
        """
    ...
    def handle_wallet_notify(self, txid):
        ...
        for address, amount in addresses.items():
            if wallet.get_account_by_address(address):
                account, transaction = wallet.receive(txid, address, amount, extra)
                logger.error("Imported account %d, address %s, amount %s, tx confirmations %d", account.id, address, amount, transaction.confirmations)
            else:
                logger.error("Skipping transaction notify for unknown address %s, amount %s", address, amount)
